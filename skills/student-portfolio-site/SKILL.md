...
1. **Invite the student's own references, then offer a tailored direction menu.** Explicitly invite them to submit screenshots or links to webpages and applications they like; ask what attracts them when they can articulate it, but do not require design vocabulary or a complete explanation. References are optional, and can be public or private. For every submitted item, respect its privacy and access boundary: do not republish a private screenshot, scrape beyond a supplied public link, or assume motion and responsive behavior that a static screenshot cannot show. Then give three genuinely distinct directions grounded in their target role, evidence, stated taste, and submitted references—not a one-size-fits-all template. For each direction, state the intended impression, page rhythm or information density, typography and color/material cues, imagery treatment, and appropriate degree of interaction. A useful set might contrast a quiet editorial portfolio, a precise professional system, a maker/developer interface, an expressive visual narrative, or a playful experimental concept, but only include directions that fit this person. Mark a recommendation when warranted without deciding for them. Always append a fourth, equally valid choice: **“我的审美比你牛，我来上传”**—or an equally playful, respectful translation in the student's language. This is not an escape hatch. If they select it, stop asking them to choose an AI-proposed direction; invite screenshots, links, a mood board, or a concise description, then let those materials lead the analysis and aesthetic brief.
2. **Analyse references and show routes after a direction is chosen.** Analyse each useful user-submitted screenshot or accessible link before implementation: identify its hierarchy, composition, type scale, spacing, palette/material, imagery, component character, and—where observable—interaction or motion. Explain which traits fit the student's goal, which do not, and why. Present 2–4 reference routes within the selected direction, combining the student's submissions with attributable public links or compact visual reference boards when useful. Make a concise mapping from each adopted principle to a planned site decision, and state what will be deliberately avoided. Treat references as inputs to an original design system: never copy another product's branding, text, proprietary imagery, distinctive assets, or page wholesale.
